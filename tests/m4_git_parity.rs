...
impl CheckpointTarget {
    fn new(operation: &str, stage: &str) -> Self {
        assert!(matches!(operation, "template" | "create" | "remove"));
...
impl CheckpointController {
    ...
    fn run_all(mut self) -> ControlRun {
        loop {
    ...
    fn crash_at_target(mut self) -> ControlRun {
        loop {
            ...
            let revalidated =
                process_identity(self.process.pid).expect("revalidate target process");
            assert_eq!(
                revalidated, self.process,
                "target process start identity changed"
            );
            let mut members =
                process_group_members(self.process.pgid).expect("inspect target group");
    ...
    fn fault_at_first(mut self, fault: ProtocolFault) -> Output {
        let checkpoint = self
            .next_checkpoint()
    ...
    fn next_checkpoint(&mut self) -> io::Result<Checkpoint> {
        ...
        if pid != self.process.pid
            ...
            || !matches!(fields[7], "template" | "create" | "remove")
